...
class Database{
    ...
    { // gotta be consistent or linters will ruin your day! =P
        if ( $this->conn ) {
            return $this->conn;
        }
        try{
    ...
    public function condition($cond, $join){
        ...
        // since we're not returning the changes to `$cond`, I think we can remove that code ;)
        foreach($cond as $col=>$val) { // no changes, just fixed indentation!
            $where_str .= empty($where_str) ? ' ' : $join; //Connecting conditions

            $where_str .= $col . "=:" . $col; //Forming prepared statement - should this be `$col . "=:" . $val` ?
    ...
    //Read - I'm not seeing any binds here! We need to fix that, OR... your app cannot accept
    // user submitted data to query from D=
